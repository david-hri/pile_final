...
def init_K(stop,gamma_0=w1/10**5,r=1):
    lT=np.array([i for i in range(stop)])*dt
    K=gamma_0*(np.cos(w1*lT)+r*np.cos(w2*lT))*np.exp(-lT**2/2/delta_T**2)
    return K



def F_frottement(stop,K,pos):
    f=0
    for i in range(min(stop,len(pos))):
        f+=K[i]*pos[-1-i]
    return f


def main_QTB(etape,N,Vprime,A,C3,K,stop):  #integration of the motion equation, 
    for i in range(N):
        f1=F_frottement(stop,K,atome[i].v1)
        f2=F_frottement(stop,K,atome[i].v2)
        #print(f1/(atome[i].v1[-1]*gamma))
        F1 = - 1/N*Vprime(atome[i].x1[-1],atome[i].x2[-1],C3)[0] - m*f1 + A[etape]
        F2 = - 1/N*Vprime(atome[i].x1[-1],atome[i].x2[-1],C3)[1] - m*f2 + A[etape]
...
def simulation_QTB(N,Vprime,A,C3,K=None,stop=None): #main function which computes the positions of the P particles in a given distribution of potential
    init(N)
    for i in range(pas):
        if i%1000000==0:
            print(i/1000000)
        main_QTB(i,N,Vprime,A,C3,K,stop)
